...
router.route('/addInstructor/:name/:course').put(auth,async(req,res)=>{
    ...
    try {
        ...
        const mydepm =await Departments.findOne({_id:user.department}) ;
        const mycourses= mydepm.courses;
        var myinstructor, mycourse;
        console.log("هل تراني ");
        ...
        const mymembers=mycourse.academicMembers;
        ...
        const mycourses= mydepm.courses;
          mycourses.forEach(async(element, index, arr) =>{  
            ...
            console.log(mycourse)
                ...
                if(index==arr.length-1)
                res.json("no course with this name iin your department");
